...
Linz.prototype.set = function(setting, val, override) {
    ...
    if (
        onceOnly.indexOf(setting) >= 0 &&
        this.settings[setting] !== undefined
    ) {
        return debugSet(
            "You can only set '" + setting + "' once. Ignoring set this time."
        );
    }
    ...
    const isObject = (obj) =>
        !Object.is(obj, null) && !Array.isArray(obj) && obj === Object(obj);

    if (updateSetting && isObject(this.settings[setting]) && isObject(val)) {
    ...
    if (
        updateSetting &&
        (!isObject(this.settings[setting]) || !isObject(val))
    ) {
...
linz.middleware = require('./lib/api/middleware');
...
Linz.prototype.configure = function() {
    ...
    if ([1, 2].indexOf(_this.mongoose.connection.readyState) < 0) {
        if (!_this.get('mongo')) {
            throw new Error(
                'You must either supply a connected mongoose object, or a mongo URI'
            );
        }
...
Linz.prototype.initConfigs = function(cb) {
    ...
    db.collection(this.get('configs collection name'), function(
        err,
        collection
    ) {
        async.each(
            Object.keys(configs),
            function(configName, initDone) {
...
Linz.prototype.defaultConfiguration = function(cb) {
    ...
    if (this.get('request logging') === true)
        routesManager.setupLoggingRoutes();
...
Linz.prototype.bootstrapExpress = function(cb) {
    ...
    if (typeof this.get('cookie parser') === 'function') {
        ...
        this.app.use(
            cookieParser(this.get('cookie secret'), this.get('cookie options'))
        );
    }
    ...
    async.parallel(
        [
            function(done) {
                ...
                var models = {
                    ...
                    permission: function(user, callback) {
                        return linz.api.permissions.hasPermission(
                            user,
                            'models',
                            'canList',
                            callback
                ...
                async.each(
                    Object.keys(linzModels),
                    function(model, callback) {
                        // get the model options
                        linzModels[model].getModelOptions(function(
                            err,
                            options
                        ) {
                            if (err) {
                            ...
                            models.children.push({
                                name:
                                    linzModels[model].linz.formtools.model
                                        .plural,
                                href:
                                    _this.get('admin path') +
                                    '/model/' +
                                    model +
                                    '/list',
            ...
            (function() {
                ...
                if (
                    typeof _this.get('navigation configuration') !== 'function'
                ) {
                    return linz.utils.noOp();
                }

                return function(done) {
                    return _this.get('navigation configuration')(nav, function(
                        err,
                        _nav
                    ) {
                        if (err) {
                            errors.log(
                                'The navigation configuration function errored.'
                            );
                            return done(null);
                        }
        ...
        function(err) {
            ...
            _this.set(
                'navigation',
                nav.sort((a, b) => collator.compare(a.name, b.name))
            );
...
Linz.prototype.logger = function(options) {
    ...
    if (!options.name)
        throw new Error(
            'You must pass the name parameter when creating a logger.'
        );
