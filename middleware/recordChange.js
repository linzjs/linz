...
const updateDate = ({ data, fieldName, theirChange, yourChange }) => {
    ...
    if (hasValue(theirChange[fieldName])) {
        data.theirChange[fieldName] = moment(
            new Date(theirChange[fieldName])
        ).format('YYYY-MM-DD');
    }

    if (hasValue(yourChange[fieldName])) {
        data.yourChange[fieldName] = moment(
            new Date(yourChange[fieldName])
        ).format('YYYY-MM-DD');
    }
};
...
const updateData = ({
    ...
    const type =
        (form[fieldName] && form[fieldName].type) ||
        schema.paths[fieldName].type;
...
const sanitiseData = (model, exclusions, yourChange, theirChange) =>
    new Promise((resolve, reject) => {
        ...
        model.schema.eachPath((fieldName) => {
            ...
            if (transposeFn) {
                let transposeResult = transposeFn(
                    theirChange[fieldName],
                    theirChange
                );

                if (!(transposeResult instanceof Promise)) {
                    transposeResult = Promise.resolve(
                        deprecate(
                            () => transposeResult,
                ...
                promises.push(
                    transposeResult.then(
                        (result) => (theirChange[fieldName] = result)
                    )
                );
            }
...
module.exports = (req, res, next) => {
    ...
    if (model.linz.formtools.form) {
        Object.keys(model.linz.formtools.form).forEach((fieldName) => {
            if (
                model.linz.formtools.form[fieldName].edit &&
                model.linz.formtools.form[fieldName].edit.disabled
            ) {
                exclusionFields[fieldName] = 0;
            }
        });
    ...
    async.waterfall(
        [
            ...
            (doc, cb) => {
                ...
                ccSettings.modifiedByCellRenderer(
                    doc[ccSettings.modifiedByProperty],
                    doc,
                    ccSettings.modifiedByProperty,
                    model,
                    (err, result) => {
                        if (err) {
                            return cb(err);
        ...
        (err, result) => {
            ...
            sanitiseData(model, exclusionFields, formData, result)
                .then(({ yourChange, theirChange }) => {
                    ...
                    if (
                        parseInt(yourChange.versionNo, 10) ===
                            parseInt(theirChange.versionNo, 10) ||
                        (req.params.versionNo &&
                            parseInt(req.params.versionNo, 10) ===
                                parseInt(theirChange.versionNo, 10))
                    ) {
                        return res.status(200).json(resData);
                    }
                    ...
                    if (
                        ...
                        (diffResult.length === 1 &&
                            diffResult[0].path &&
                            diffResult[0].path[0] === 'modifiedBy')
                    ) {
                    ...
                    diffResult.forEach((diff) => {
                        ...
                        let field =
                            (model.form && model.form[fieldName]) ||
