...
module.exports = function(req) {
    ...
    if (Model.versions.compare && Model.versions.compare.exclusions) {
        ...
        Object.keys(exclusions).forEach(
            (fieldName) => (fieldExclusions[fieldName] = exclusions[fieldName])
        );
    }

    var getAuthor = (record, cb) => {
        Model.versions.cellRenderers.referenceName(
            record.modifiedBy,
            record,
            'modifiedBy',
            Model,
    ...
    var getDate = (record, cb) => {
        Model.versions.cellRenderers.date(
            record.dateCreated,
            record,
            'dateCreated',
            Model,
    ...
    var getReferenceNames = (record, cb) => {
        async.each(
            Object.keys(record),
            (fieldName, getReferenceDone) => {
                const defaultRenderer =
                    Model.versions.cellRenderers.default ||
                    linz.versions.renderers.cellRenderers.default;
    ...
    var getVersionById = (id, projection, bRenderFields, cb) => {
        Model.VersionedModel.findById(id, projection, { lean: 1 })
            .exec()
            .then((record) => {
                if (!record) {
                ...
                getReferenceNames(record, (err, record) =>
                    cb(null, getFieldNames(record))
                );
            })
            .catch((err) => cb(err));
    };
    ...
    return {
        getLastest: (cb) =>
            getVersionById(req.params.revisionBId, fieldExclusions, false, cb),

        getPrevious: (cb) =>
            getVersionById(req.params.revisionAId, fieldExclusions, false, cb),
