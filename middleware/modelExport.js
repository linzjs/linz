...
const promisify = (fn, callWith = []) => () =>
    new Promise((resolve, reject) => {
        fn.apply(
            null,
            callWith.concat((err, form) => {
                if (err) {
                    return reject(err);
                }
...
var getExport = function(exports) {
    ...
    if (!exp) {
        throw new Error(
            "The export was using Linz default export method, yet the model's list.export object could not be found."
        );
    }
        ...
        Promise.all([getLabels(), getList()])
            .then(([labels, list]) => {
                ...
                const includedFields =
                    req.linz.export.inclusions.length > 0
                        ? req.linz.export.inclusions.split(',')
                        : getInclusionsFromExclusions(
                              req.linz.export,
                              req.linz.model
                          );

                const fieldLabels = {};
                const includedPaths = Object.keys(
                    req.linz.model.schema.paths
                ).filter((key) => includedFields.includes(key));

                includedPaths.forEach(function(field) {
                    if (!labels[field]) {
                        throw new Error(
                            `Could not find ${field} field in the labels object.`
                        );
                    }
    ...
    post: function(req, res, next) {
        ...
        return Promise.all([
            ...
            .then(([form, labels, list, filters]) => {
                ...
                const includedFields =
                    exportObj.inclusions.length > 0
                        ? exportObj.inclusions.split(',')
                        : getInclusionsFromExclusions(exportObj, Model);
                const bodyFields = req.body.selectedFields.split(',');
                const refFieldNames = [];
                const fields = includedFields.filter((field) =>
                ...
                req.linz.model.getQuery(req, filters, function getQuery(
                    err,
                    query
                ) {
                    if (err) {
                    ...
                    req.linz.model.listQuery(
                        req,
                        query,
                        (listQueryErr, listQuery) => {
                            if (listQueryErr) {
                                return next(listQueryErr);
                            }

                            const exportQuery = listQuery.select(
                                filterFieldNames.join(' ')
                            );
                            const columnsFn =
                                exportObj.columns || ((columns) => columns);
                            ...
                            linz.api.util.generateExport({
                                ...
                                name: `${
                                    Model.linz.formtools.model.plural
                                }-${moment(Date.now())
                                    .format('l')
                                    .replace(/\//g, '.', 'g')}`,
                                req,
