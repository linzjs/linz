...
module.exports = function(req, res, next) {
    ...
    function handleQueryError(err) {
        ...
        if (queryErrorCount > 1) {
            ...
            req.linz.notifications.push(
                linz.api.views.notification({
                    text:
                        'One of the filters is repeatedly generating an error. All filters have been removed.',
                    type: 'error',
                })
            );
        ...
        req.linz.notifications.push(
            linz.api.views.notification({
                text:
                    'An error has occured with one of the filters you added. It has been removed.',
                type: 'error',
            })
        );
    ...
    function getModelIndex() {
        ...
        pageSize = Number(
            session.list.formData.pageSize || req.linz.model.list.paging.size
        );
        ...
        async.series(
            [
                ...
                function(cb) {
                    formtoolsAPI.list.renderToolbarItems(
                        req,
                        res,
                        model,
                        function(err, result) {
                ...
                function(cb) {
                    ...
                    formtoolsAPI.list.renderFilters(req, model, function(
                        err,
                        result
                    ) {
                        if (err) {
                ...
                function(cb) {
                    ...
                    const alwaysOnWithDefault = Object.keys(filters).filter(
                        (key) =>
                            filters[key].alwaysOn === true &&
                            Object.keys(filters[key]).includes('default')
                    );
                    ...
                    if (
                        ...
                        formData.selectedFilters.length
                    ) {
                ...
                function(cb) {
                    ...
                    formtoolsAPI.list.getActiveFilters(
                        req,
                        session.list.formData.selectedFilters.split(','),
                        session.list.formData,
                        model,
                        function(err, result) {
                            if (err) {
                ...
                function(cb) {
                    ...
                    formtoolsAPI.list.renderSearchFilters(
                        req,
                        session.list.formData.selectedFilters.split(','),
                        session.list.formData,
                        model,
                ...
                function(cb) {
                    if (
                        !session.list.formData.search ||
                        !session.list.formData.search.length ||
                        !req.linz.model.list.search ||
                        !Array.isArray(req.linz.model.list.search)
                    ) {
                        return cb(null);
                    }
                ...
                // create the query
                function(cb) {
                    req.linz.model.getQuery(req, filters, function(
                        err,
                        result
                    ) {
                        if (err) {
                ...
                // get page total
                function(cb) {
                    req.linz.model.getCount(req, query, function(err, count) {
                        if (err) {
                            return cb(err);
                        }
                ...
                function(cb) {
                    ...
                    if (
                        !session.list.formData.sort &&
                        req.linz.model.list.sortBy.length
                    ) {
                        req.linz.model.list.sortingBy =
                            req.linz.model.list.sortBy[0];

                        // set default form sort
                        session.list.formData.sort = `${getDefaultOrder(
                            req.linz.model.list.sortingBy
                        )}${req.linz.model.list.sortingBy.field}`;
                    } else {
                        req.linz.model.list.sortBy.forEach(function(sort) {
                            if (
                                sort.field === session.list.formData.sort ||
                                '-' + sort.field === session.list.formData.sort
                            ) {
                ...
                function(cb) {
                    ...
                    async.each(
                        Object.keys(mongooseRecords),
                        function(index, recordDone) {
                            mongooseRecords[index].canEdit(req, function(
                                err,
                                result,
                                message
                            ) {
                ...
                function(cb) {
                    ...
                    async.each(
                        Object.keys(mongooseRecords),
                        function(index, recordDone) {
                            mongooseRecords[index].canDelete(req, function(
                                err,
                                result,
                                message
                            ) {
                ...
                function(cb) {
                    ...
                    for (let field in req.linz.model.list.fields) {
                        ...
                        if (
                            req.linz.model.schema.tree[field] &&
                            req.linz.model.schema.tree[field].ref
                        ) {
                            // Get the records.
                            ...
                            // Now get the values.
                            refColData[field].values = refColData[
                                field
                            ].records.map((record) =>
                                linz.api.model.getObjectIdFromRefField(
                                    record[field]
                    ...
                    async.each(
                        ...
                        function(field, fieldDone) {
                            ...
                            linz.formtools.cellRenderers.reference.apply(
                                null,
                                args
                            );
                        },
                ...
                function(cb) {
                    ...
                    async.each(
                        ...
                        function(index, recordDone) {
                            ...
                            async.each(
                                Object.keys(req.linz.model.list.fields),
                                function(field, fieldDone) {
                                    ...
                                    if (
                                        !req.linz.model.list.fields[field]
                                            .virtual
                                    ) {
                ...
                function(cb) {
                    ...
                    async.each(
                        req.linz.model.list.recordActions,
                        function(action, actionDone) {
                            ...
                            if (typeof action.disabled !== 'function') {
                                throw new Error(
                                    'Invalid type for record.action.disabled. It must be a function'
                                );
                            }
                            ...
                            async.each(
                                ...
                                function(index, recordDone) {
                                    action.disabled(
                                        ...
                                        function(err, isDisabled, message) {
                                            records[index].recordActions =
                                                records[index].recordActions ||
                                                {};
