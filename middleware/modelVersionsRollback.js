        ...
        async.series(
            {
                ...
                previous: function(cb) {
                    ...
                    if (
                        Model.versions.ignorePaths &&
                        Model.versions.ignorePaths.length
                    ) {
            ...
            function(err, result) {
                ...
                if (!result.latest || !result.previous) {
                    return new Error(
                        'Error: Unable to find one of the records required for the rollback operation.'
                    );
                }
                ...
                if (
                    Model.getModifiedByFieldName &&
                    Model.schema.path(Model.getModifiedByFieldName())
                ) {
