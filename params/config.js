...
module.exports = function(router) {
    ...
    router.param('config', function(req, res, next, configName) {
        ...
        async.parallel(
            [
                function(cb) {
                    req.linz.config.linz.formtools.labels = linz.api.configs.labels(
                        configName
                    );
                ...
                function(cb) {
                    linz.api.configs.permissions(req.user, configName, function(
                        err,
                        permissions
                    ) {
                        if (err) {
                ...
                function(cb) {
                    linz.api.configs.overview(req, configName, function(
                        err,
                        overview
                    ) {
                        if (err) {
                ...
                function(cb) {
                    ...
                    async.forEachOf(
                        ...
                        function(field, key, callback) {
                            ...
                            // Retrieve the form.
                            field.schema.statics.getForm(req.user, function(
                                err,
                                embeddedForm
                            ) {
                                if (embeddedForm) {
