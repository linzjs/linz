...
beforeAll((done) => {
    ...
    linz.init({
        options: {
            'mongo': 'mongodb://mongodb:27017/api-url-test',
            'user model': 'user',
            'load models': false,
            'load configs': false,
        },
...
describe('Linz has a url api', () => {
    describe('has a getAdminLink method', () => {
        ...
        describe('on models', () => {
            test('it should allow a list action', () => {
                expect(
                    linz.api.url.getAdminLink(linz.mongoose.model('user'))
                ).toBe('/admin/model/user/list');
                expect(
                    linz.api.url.getAdminLink(
                        linz.mongoose.model('user'),
                        'list'
                    )
                ).toBe('/admin/model/user/list');
            });
            ...
            test('it should allow a create action', () => {
                ...
                ).toBe('/admin/model/user/create');
            });

            test('it should return the model listing url', () => {
                expect(linz.api.url.getAdminLink(undefined, 'list')).toBe(
                    '/admin/models/list'
                );
            });
        });
        ...
        describe('on records', () => {
            test('it should allow an overview action', () => {
                ...
                ).toBe('/admin/model/user/id/overview');
                expect(
                    linz.api.url.getAdminLink(
                        linz.mongoose.model('user'),
                        'overview',
            ...
            test('it should allow a delete action', () => {
                ...
                ).toBe('/admin/model/user/id/delete');
            });
        });
        ...
        describe('on configs', () => {
            ...
            test('it should allow an overview action', () => {
                expect(linz.api.url.getAdminLink(config, undefined, 'id')).toBe(
                    '/admin/config/id/overview'
                );
                expect(
                    linz.api.url.getAdminLink(config, 'overview', 'id')
                ).toBe('/admin/config/id/overview');
            });

            test('it should allow an edit action', () => {
                expect(linz.api.url.getAdminLink(config, 'edit', 'id')).toBe(
                    '/admin/config/id/edit'
                );
            });

            test('it should allow a save action', () => {
                expect(linz.api.url.getAdminLink(config, 'save', 'id')).toBe(
                    '/admin/config/id/save'
                );
            });

            test('it should allow a delete action', () => {
                expect(linz.api.url.getAdminLink(config, 'delete', 'id')).toBe(
                    '/admin/config/id/delete'
                );
            });

            test('it should return the config listing url', () => {
                expect(linz.api.url.getAdminLink(config, 'list')).toBe(
                    '/admin/configs/list'
                );
            });
        });
    ...
    describe('has a getLink method', () => {
        ...
        test('it should generate url with multiple query parameters', () => {
            expect(linz.api.url.getLink('one', 'two', 'three')).toBe(
                '/admin/one/two/three'
            );
        });
    });
