    ...
    async.waterfall(
        [
            async function(cb) {
                ...
                Object.keys(req.linz.config.schema.paths).forEach(function(
                    field
                ) {
                    if (field !== '_id' && req.body[field] !== undefined) {
                        ...
                        if (
                            formUtils.schemaType(
                                req.linz.config.schema.paths[field]
                            ) === 'documentarray'
                        ) {
                            ...
                            documents.forEach(function(doc) {
                                ...
                                req.linz.config.schema.paths[
                                    field
                                ].schema.eachPath(function(fieldName, field) {
                                    // if the field hasn't come through the form, apply the default
                                    if (doc[fieldName] === undefined) {
                                        doc[
                                            fieldName
                                        ] = linz.formtools.utils.getDefaultValue(
                                            field
                                        );
                                    }
                                });
                            ...
                            if (form[field].type === 'boolean') {
                                req.body[field] = utils.asBoolean(
                                    req.body[field]
                                );
                            }

                            // go through the transform function if one exists
                            record[field] = form[field].transform
