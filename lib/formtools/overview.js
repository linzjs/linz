...
function renderFields(req, res, record, model, section, callback) {
    ...
    var fields = section.fields.filter(function (field) {
        ...
        return field.label ? field : false;
    ...
    async.mapSeries(fields, function (field, cb) {
        ...
        if (typeof field === 'object') {
            ...
            return field.value(req, res, record, model, (fieldValueErr, result) => {
        ...
        var formField = model.linz.formtools.form[field],
            renderer = cellRenderers[formField.type] ? formField.type : (model.schema.tree[field] && model.schema.tree[field].ref) ? 'reference' : (record[field] && linz.mongoose.models[record[field].ref]) ? 'referenceValue' : 'text';
...
function getSections (req, res, record, model, section, labelRequired, cb) {
    ...
    if (Array.isArray(section.body)) {
        ...
        return getSections(req, res, record, model, section.body, false, function (err, result) {
    ...
    async.mapSeries(section, function (obj, callback) {
        ...
        if (labelRequired && !obj.label) {
            return cb(new Error('One of the label is missing in overview of ' + model.modelName + ' model.'));
        }
...
function body (req, res, record, model, cb) {
    ...
    async.mapSeries(overview.body, function (section, callback) {
        ...
        return getSections(req, res, record, model, section, true, callback);
