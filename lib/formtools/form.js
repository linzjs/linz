...
var generateFormFromModel = exports.generateFormFromModel = function (m, schemaFields, r, formType, cb) {
    ...
    async.eachSeries(Object.keys(formFields), function (fieldName, fn) {
        ...
        var fieldType = (field.type && field.type.name && field.type.name.toLowerCase()) || field.type;
        var formField;
        var choices = {};
        ...
        async.series([
            ...
            function (callback) {
                ...
                if (field.list || !m.tree[fieldName] || !m.tree[fieldName].ref) {
                    return callback(null);
                }
            ...
            function (callback) {
                ...
                if (getTransposeFn(formFields, fieldName, 'form')) {
                    ...
                    fieldValue = getTransposeFn(formFields, fieldName, 'form')(r[fieldName], r, 'form');
                    ...
                    switch (fieldType) {
                        ...
                        case 'date':
                            fieldValue = Promise.resolve(r[fieldName] !== undefined && r[fieldName] !== null ? moment(r[fieldName]).toISOString() : defaultValue);
                        break;

                        case 'boolean':
                            fieldValue = Promise.resolve((r[fieldName] !== undefined) ? utils.asBoolean(r[fieldName]) : defaultValue);
                        break;

                        case 'array':
                            fieldValue = Promise.resolve((r[fieldName] !== undefined) ? r[fieldName] : defaultValue);
                        break;
                        ...
                        default:
                            fieldValue = Promise.resolve((r[fieldName] !== undefined && r[fieldName] !== null ? r[fieldName].toString() : null) || (defaultValue !== null ? defaultValue.toString() : defaultValue));
                        break;
                ...
                if (!(fieldValue instanceof Promise)) {
                    fieldValue = Promise.resolve(deprecate(() => fieldValue, 'Transposing a field without returning a promise has been deprecated.')());
                }
                    ...
                    fieldValue
                        .then((val) => {
                            ...
                            switch (fieldType) {
                                ...
                                case 'documentarray':
                                    w = wFn(fieldName, field, val, r, formType, embeddedDocumentForm);
                                    break;

                                default:
                                    w = wFn(fieldName, field, val, r, formType);
                                    break;
