...
var gridSettings = function gridSettings (schema, grid) {

    for (var key in grid.columns) {
        // if a column exists in grid columns but not in schema, that means this is a virtual column
        if (!schema.paths[key]) {

            if (typeof grid.columns[key].renderer === 'function' && grid.columns[key].renderer.name !== 'defaultRenderer') {
               grid.columns[key].virtual = true;
            }
...
var plugin = module.exports = function formtoolsPlugin (schema, options) {
    ...
    // load grid settings
    grid = gridSettings(schema, options.grid);
