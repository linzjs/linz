...
module.exports = function(list, multiple, options) {
    ...
    if (!Array.isArray(list) && typeof list !== 'function') {
        throw new Error(
            'List paramenter must either be an array or a function.'
        );
    }

    var renderHTML = function renderHTML(fieldName, data, form) {
        var dataAttributes = Object.keys(settings.data)
            .map(function(key) {
                return 'data-' + key + '="' + settings.data[key] + '"';
            })
            .join(' ');
        var html =
            '<select name="' +
        ...
        if (data) {
            data.forEach(function(obj) {
                ...
                if (form && form[fieldName]) {
                    const values = form[fieldName].map((value) =>
                        utils.parseString(value, {
                            parseNumber: settings.parseNumber,
                        })
                    );

                    selected =
                        values.indexOf(obj.value) >= 0 ? ' selected' : '';
                }

                html +=
                    '<option value="' +
                    linz.api.util.escape(obj.value) +
                    '"' +
                    selected +
                    '>' +
                    linz.api.util.escape(obj.label) +
                    '</option>';
            });
        }
