...
module.exports = (dateFormat = 'YYYY-MM-DD') => ({
    ...
    bind: function dateRangeFilterBinder (fieldName, form, callback) {
        ...
        for (var i = 0; i < form[fieldName].dateFrom.length; i++) {
            ...
            html.push('<input type="text" name="' + fieldName + '[dateFrom][]" class="form-control" data-ui-datepicker="true" data-linz-date-format="' + dateFormat + '" data-linz-date-value="' + linz.api.util.escape(dateFromIso) + '" required><input type="text" name="' + fieldName + '[dateTo][]" class="form-control" data-ui-datepicker="true" data-linz-date-format="' + dateFormat + '" data-linz-date-value="' + linz.api.util.escape(dateToIso) + '" required>');
