...
module.exports = (dateFormat = 'YYYY-MM-DD') => ({
    ...
    renderer: function dateRangeFilterRenderer (fieldName, callback) {
        return callback(null, '<template><input type="text" name="' + fieldName + '[dateFrom][]" class="form-control" style="width:50%;" data-ui-datepicker="true" data-linz-date-format="' + dateFormat + '" required><input type="text" name="' + fieldName + '[dateTo][]" class="form-control" style="width:50%;" data-ui-datepicker="true" data-linz-date-format="' + dateFormat + '" required></template>');
    },
