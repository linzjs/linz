...
module.exports = {
    renderer: function dateRangeFilterRenderer (fieldName, callback) {
        return callback(null, '<input type="date" name="' + fieldName + '[dateFrom][]" class="form-control" style="width:50%;" required><input type="date" name="' + fieldName + '[dateTo][]" class="form-control" style="width:50%;" required>');
    },
    ...
    bind: function dateRangeFilterBinder (fieldName, form, callback) {
        ...
        for (var i = 0; i < form[fieldName].dateFrom.length; i++) {
            ...
            html.push('<input type="date" name="' + fieldName + '[dateFrom][]" class="form-control" style="width:50%;" value="' + dateFrom + '" required><input type="date" name="' + fieldName + '[dateTo][]" class="form-control" style="width:50%;" value="' + dateTo + '" required>');
