...
module.exports = (dateFormat = 'YYYY-MM-DD') => ({
    ...
    bind: function dateRangeFilterBinder (fieldName, form, callback) {
        ...
        for (var i = 0; i < form[fieldName].dateFrom.length; i++) {
            ...
            html.push('<span><input type="text" name="' + fieldName + '[dateFrom][]" class="form-control" style="width:50%;" value="' + dateFrom + '" data-ui-datepicker="true" data-linz-date-format="' + dateFormat + '" required></span><span><input type="text" name="' + fieldName + '[dateTo][]" class="form-control" style="width:50%;" value="' + dateTo + '" data-ui-datepicker="true" data-linz-date-format="' + dateFormat + '" required></span>');
