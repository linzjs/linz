...
module.exports = (dateFormat = 'YYYY-MM-DD') => ({
    renderer: function dateRangeFilterRenderer(fieldName, callback) {
        return callback(
            ...
            '<template><input type="text" name="' +
                fieldName +
                '[dateFrom][]" class="form-control" data-ui-datepicker="true" data-linz-date-format="' +
                dateFormat +
                '" required><input type="text" name="' +
                fieldName +
                '[dateTo][]" class="form-control" data-ui-datepicker="true" data-linz-date-format="' +
                dateFormat +
                '" required></template>'
        );
    },
    ...
    filter: function dateRangeFilterFilter(fieldName, form, callback) {
        if (
            !form[fieldName].dateFrom ||
            !form[fieldName].dateTo ||
            form[fieldName].dateFrom.length <= 0 ||
            form[fieldName].dateTo <= 0
        ) {
        ...
        if (selectedDatesFrom.length === 1) {
            if (
                !moment(selectedDatesFrom[0], dateFormat).isValid() ||
                !moment(selectedDatesTo[0], dateFormat).isValid()
            ) {
            ...
            obj[fieldName] = dateRangeQuery(
                selectedDatesFrom[0],
                selectedDatesTo[0],
                dateFormat,
                form.linzTimezoneOffset
            );
        ...
        for (var i in selectedDatesFrom) {
            if (
                !moment(selectedDatesFrom[i], dateFormat).isValid() ||
                !moment(selectedDatesTo[i], dateFormat).isValid()
            ) {
                return callback(new Error('One of the dates is invalid'));
            }
    ...
    bind: function dateRangeFilterBinder(fieldName, form, callback) {
        ...
        for (var i = 0; i < form[fieldName].dateFrom.length; i++) {
            dateFrom = form[fieldName].dateFrom[i]
                ? form[fieldName].dateFrom[i]
                : '';
            ...
            html.push(
                '<input type="text" name="' +
                    ...
                    '[dateFrom][]" class="form-control" data-ui-datepicker="true" data-linz-date-format="' +
                    dateFormat +
                    '" data-linz-date-value="' +
                    linz.api.util.escape(dateFromIso) +
                    '" required><input type="text" name="' +
                    fieldName +
                    '[dateTo][]" class="form-control" data-ui-datepicker="true" data-linz-date-format="' +
                    dateFormat +
                    '" data-linz-date-value="' +
                    linz.api.util.escape(dateToIso) +
                    '" required>'
            );
        }
