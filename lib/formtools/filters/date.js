...
module.exports = (dateFormat = 'YYYY-MM-DD') => ({
        ...
        filter: function dateFilterFilter (fieldName, form, callback) {
            ...
            if (typeof form[fieldName] === 'string') {
                selectedDates = form[fieldName].split(',');
            }
        ...
        bind: function dateFilterBinder (fieldName, form, callback) {
            ...
            form[fieldName].forEach(function (dateString) {
                ...
                html.push('<input type="text" name="' + fieldName + '[]" class="form-control" data-ui-datepicker="true" data-linz-date-format="' + dateFormat + '" data-linz-date-value="' + linz.api.util.escape(value) + '" required>');
