...
module.exports = {
    ...
    renderer: function dateFilterRenderer (fieldName, callback) {
        return callback(null, '<input type="date" name="' + fieldName + '[]" class="form-control">');
    },
    ...
    bind: function dateFieldBinder (fieldName, form, callback) {

        var html = [];

        form[fieldName].forEach(function (val) {
            html.push('<input type="date" name="' + fieldName + '[]" class="form-control" value="' + val + '">');
        });
