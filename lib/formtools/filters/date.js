...
module.exports = {
    ...
    renderer: function dateFilterRenderer (fieldName, callback) {
        return callback(null, '<template><input type="date" name="' + fieldName + '[]" class="form-control" data-ui-datepicker="true" required></template>');
    },
    ...
    bind: function dateFilterBinder (fieldName, form, callback) {
        ...
        form[fieldName].forEach(function (val) {
            html.push('<input type="date" name="' + fieldName + '[]" class="form-control" value="' + val + '" data-ui-datepicker="true" required>');
        });
