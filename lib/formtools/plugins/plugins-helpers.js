...
var gridDefaults = function (grid, labels) {

    // defaults to {}
    grid = grid || {};

    // defaults to []
    grid.actions = actionsDefaults(grid.actions) || [];

    // defaults to []
    grid.toolbarItems = grid.toolbarItems || [];

    // defaults to true
    grid.showSummary = !(grid.showSummary === false);

    grid.sortBy = transformSortBy(labels, grid.sortBy || ['dateModified']);

    // define a default set of columns
    grid.columns = columnsDefaults(grid.columns, labels) || {
        title: {
            label: 'Label',
    ...
    grid.filters = filtersDefault(grid.filters, labels) || {};

    grid.paging = grid.paging || {};

    // defaults to true
    grid.paging.active = !(grid.paging.active === false);

    // defaults to 20
    grid.paging.size = grid.paging.size || linz.get('page size');

    // detaults to [20,50,100,200]
    grid.paging.sizes = grid.paging.sizes || linz.get('page sizes');

    // defaults to []
    grid.groupActions = actionsDefaults(grid.groupActions) || [];

    // defaults to []
    grid.recordActions = actionsDefaults(grid.recordActions) || [];

    // defaults to []
    grid.export = exportDefault(grid.export) || [];
...
var overviewDefaults = function (overview, labels, schema, form) {
    ...
    overview.summary = overviewSummaryDefault(overview.summary) || {
        fields: {
            dateCreated: {
                label: 'Date created',
                renderer: linz.formtools.cellRenderers.date
    ...
    Object.keys(overview.summary.fields).forEach(function (fieldName) {
        overview.summary.fields[fieldName].label = overview.summary.fields[fieldName].label || labels[fieldName];
    });

    overview.summary.label = overview.summary.label || 'Summary';


    //overview Details
    if (!overview.details || typeof overview.details !== 'object') {
        // defaults to {}
        overview.details = {};
    }

    overview.details.form = overviewDetailsDefault(overview.details.fields, schema, form, labels);

    //set tabs to tabsArray property of overview.details
    var tabsArray = [];
    if (Array.isArray(overview.details.tabs) && overview.details.tabs.length) {

        for (var i = 0; i < overview.details.tabs.length; i++) {

            var tempTab = {};
            tempTab.label = overview.details.tabs[i].label;

            if (Array.isArray(overview.details.tabs[i].fields)) {

                tempTab.form = overviewDetailsDefault(overview.details.tabs[i].fields, schema, form, labels);

            } else if (typeof overview.details.tabs[i].body === 'function') {
                tempTab.body = overview.details.tabs[i].body;
            }

            tabsArray.push(tempTab);
        }
    }

    //set the tabsArray to overview.details.tabsArray
    overview.details.tabsArray = tabsArray;

    overview.details.label = overview.details.label || 'Detail';
...
var overviewDetailsDefault = function (detail, schema, form, labels) {

    if (!Array.isArray(detail)) {
        return {};
    }

    //the array passed to detail argument is expected to contain names of form fields
    //so, make sure the array contains only string values
    var fieldsArray = detail.filter( function (val) {
    ...
    return formSettings(schema, form, labels, fieldsArray);

};

var formSettings = function (schema, form, labels, detailFields) {
    ...
    if (detailFields && detailFields.length) {
        ...
        var detailForm = detailFields.filter(function (val) {
            return form[val] ? true : false;
        });

        // sort form configs in the order specified in form
        detailForm.forEach( function(elm) {
            formConfigs[elm].disabled = true;
            formConfigs[elm].create.disabled = true;
            formConfigs[elm].edit.disabled = true;
            sortedFormConfigs[elm] = formConfigs[elm];
        });
...
module.exports = {
    ...
    overviewDetailsDefault: overviewDetailsDefault,
