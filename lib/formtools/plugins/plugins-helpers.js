...
var defaults = function(options, schema, isEmbeddedDocument = false) {
    options = options || {};

    /**
     * label defaults
     */
    options.labels = labelDefaults(
        options.labels || {},
    ...
    if (options.model.plural === undefined) {
        options.model.plural =
            options.model.label.length > 0
                ? inflection.pluralize(options.model.label)
                : '';
    }
...
var labelDefaults = function(labels, schemaPaths = []) {
    const providedLabels = Object.keys(labels);
    const updatedLabels = Object.assign({}, labels);

    // Default the labels
    schemaPaths.forEach((path) => {
        // If a value hasn't already been provided, default it.
        if (!providedLabels.includes(path)) {
            updatedLabels[path] = sentenceCase(path);
        }
    });
...
var listDefaults = function(list, labels, titleField) {
    // defaults to {}
    list = list || {};
    ...
    list.fields = fieldsDefaults(list.fields, labels, titleField) || {
        title: {
            label: 'Title',
    ...
    list.search = Object.keys(list).includes('search')
        ? list.search
        : ['title'];
...
var fieldsDefaults = function(fields, labels, titleField) {
    ...
    Object.keys(fields).forEach(function(field) {
        ...
        if (typeof fields[field] === 'boolean' && fields[field] === true) {
            ...
            if (field === 'title') {
                fields[field] = {
                    label: labels[field] || labels[titleField] || 'Title',
                    renderer:
        ...
        if (fields[field].virtual && !fields[field].renderer) {
            throw new Error(
                'Renderer attribute is missing for virtual field options.list.fields.' +
                    field
            );
        }

        if (field === 'title' && !fields[field].renderer) {
            fields[field].renderer = linz.formtools.cellRenderers.overviewLink;
        } else {
            // default the renderer
            fields[field].renderer =
                fields[field].renderer || linz.formtools.cellRenderers.default;
        }
    });
...
var overviewDefaults = function(overview) {
    ...
    // defaults to []
    overview.footerActions = actionsDefaults(overview.footerActions) || [];
    ...
    if (typeof overview.body !== 'function' && !Array.isArray(overview.body)) {
        throw new Error(
            'overview.body in model must be a function or an Array.'
        );
    }
...
var filtersDefault = function(filters, labels) {
    ...
    Object.keys(filters).forEach(function(key) {
        ...
        filters[key].filter =
            filters[key].filter || linz.formtools.filters.default;

        if (filters[key].filter) {
            if (!filters[key].filter.renderer) {
                throw new Error(
                    'Renderer function is missing for options.list.filters.' +
                        key
                );
            }
            ...
            if (!filters[key].filter.filter) {
                throw new Error(
                    'Filter function is missing for options.list.filters.' + key
                );
            }

            if (!filters[key].filter.bind) {
                throw new Error(
                    'Bind function is missing for options.list.filters.' + key
                );
            }
        }
    });
...
var exportDefault = function(exports) {
    ...
    exports.forEach(function(exp) {
        ...
        if (exp.action && exp.action === 'export') {
            throw new Error(
                'You can not define a model export with an action property of export, this is reserved for Linz.'
            );
        }
        ...
        var expObj = {
            ...
            exclusions:
                exp.exclusions ||
                '_id,dateCreated,dateModified,createdBy,modifiedBy',
...
var formSettings = function(req, schema, form, labels) {
    // convert schema to form configs DSL
    var formConfigs = merge(convertSchemaToFormConfig(schema), form);
    ...
    for (var key in formConfigs) {
        ...
        formConfigs[key] = formConfigDefault(
            key,
            schemaConfig,
            formConfig,
            labels
    ...
    return linz.api.formtools.decorateDocumentArrayFields(
        req,
        sortedFormConfigs
    );
};
...
var formConfigDefault = function(keyName, schemaConfig, formConfig, labels) {
    ...
    obj.create.visible =
        formConfig.create.visible === undefined
            ? obj.visible
            : formConfig.create.visible;
    obj.create.disabled =
        formConfig.create.disabled === undefined
            ? obj.disabled
            : formConfig.create.disabled;
    ...
    obj.create.required =
        formConfig.create.required === undefined
            ? obj.required
            : formConfig.create.required;
    obj.create.placeholder =
        formConfig.create.placeholder || formConfig.placeholder;
    obj.create.query = formConfig.create.query || {};
    obj.create.transform = formConfig.create.transform || formConfig.transform;

    // default the required properties for query
    obj.create.query.filter =
        obj.create.query.filter || obj.query.filter || undefined;
    obj.create.query.sort =
        obj.create.query.sort || obj.query.sort || undefined;
    obj.create.query.select =
        obj.create.query.select || obj.query.select || undefined;
    obj.create.query.label =
        obj.create.query.label || obj.query.label || undefined;

    obj.edit = {};
    obj.edit.label = formConfig.edit.label || obj.label;
    obj.edit.visible =
        formConfig.edit.visible === undefined
            ? obj.visible
            : formConfig.edit.visible;
    obj.edit.disabled =
        formConfig.edit.disabled === undefined
            ? obj.disabled
            : formConfig.edit.disabled;
    obj.edit.fieldset = formConfig.edit.fieldset || formConfig.fieldset;
    obj.edit.widget = formConfig.edit.widget || formConfig.widget;
    obj.edit.required =
        formConfig.edit.required === undefined
            ? obj.required
            : formConfig.edit.required;
    obj.edit.placeholder =
        formConfig.edit.placeholder || formConfig.placeholder;
    obj.edit.query = formConfig.edit.query || {};
    obj.edit.transform = formConfig.edit.transform || formConfig.transform;

    // default the required properties for query
    obj.edit.query.filter =
        obj.edit.query.filter || obj.query.filter || undefined;
    obj.edit.query.sort = obj.edit.query.sort || obj.query.sort || undefined;
    obj.edit.query.select =
        obj.edit.query.select || obj.query.select || undefined;
