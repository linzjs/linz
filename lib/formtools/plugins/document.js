...
module.exports = function formtoolsPlugin(schema, options) {
    var types = linz.mongoose.Schema.Types,
        opts = pluginHelpers.defaults(clone(options), schema),
        labels = opts.labels || {},
        list,
        overview,
        indexes = [];
    ...
    if (
        !schema.paths.title &&
        opts.model.title &&
        !Object.keys(schema.paths).includes(opts.model.title)
    ) {
        throw new Error(
    ...
    dedupe(indexes).forEach((fieldName) =>
        schema.index(pluginHelpers.indexObj(fieldName))
    );

    schema.pre('save', function() {
        // Add the ability to by pass the pre-save hooks. Defaults to true.
        var bPreSave = !(this.modelPreSave === false);
    ...
    schema.statics.getForm = function(req, cb) {
        ...
        return formFn()
            .then((formObj) =>
                pluginHelpers.formSettings(req, schema, clone(formObj), labels)
            )
    ...
    schema.statics.getOverview = function(req, cb) {
        ...
        opts.overview(req, function(err, customOverview) {
            ...
            return cb(
                null,
                pluginHelpers.overviewDefaults(
                    pluginHelpers.overviewDefaults(clone(customOverview))
                )
            );
        });
    };
    ...
    schema.statics.getCount = function(req, query, cb) {
        return cb(null, this.countDocuments(query));
    };
