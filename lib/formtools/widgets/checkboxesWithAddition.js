...
class CheckboxesWithAddition extends Widget {
    ...
    static render (name, field, value, record, formType, attrs) {
        ...
        (field.list || []).forEach(function (f) {
            ...
            var el = new formist.Field('input', {
                ...
                attributes: {
                    ...
                    name: name,
                    value: f.value,
                    'data-linz-conflict-handler': 'checkboxesWithAdditionConflictHandler'
                },
                theme: {
                    field: function (label, content, helpText, field) {

                        // we want to append to the attributes, so default if it wasn't passed in
                        return '<div class="checkbox"><label>' + content + ' <span>' + field.options.label.label + '</span></label></div>';
        ...
        var el = new formist.Field('input', {
            ...
            theme: {
                field: function (fieldLabel, content, helpText, fieldObj) {
                    var html = '<div class="row"><div class="col-lg-3"><div class="input-group">'
                                    + '<template><div><div class="checkbox"><label>' + content + ' <span class="checkbox-label">' + fieldObj.options.label.label + '</span></label></div></div></template>'
...
module.exports = CheckboxesWithAddition;
