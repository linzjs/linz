...
const reference = (val, record, fieldName, model, callback) => {
    ...
    const generateLink = doc => `<a href="${linz.api.url.getAdminLink(refModel, 'overview', doc._id.toString())}" title="${linz.api.util.escape(doc.title)}">${linz.api.util.escape(doc.title)}</a>`;
    ...
    if (Array.isArray(val)) {
        ...
        return refModel.find({ _id: { $in: val } })
            .then((docs) => {
                ...
                docs.forEach(doc => (results[doc._id.toString()] = generateLink(doc)));
