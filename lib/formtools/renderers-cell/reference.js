...
const reference = (val, record, fieldName, model, callback) => {
    ...
    if (Array.isArray(val)) {
        ...
        return refModel.find({ _id: { $in: val } })
            .then((docs) => {
                ...
                docs.forEach(doc => (results[doc._id.toString()] = `<a href="${linz.api.url.getAdminLink(refModel, 'overview', doc._id.toString())}" title="${doc.title}">${doc.title}</a>`));
    ...
    return refModel.findById(val)
        .then(doc => callback(null, `<a href="${linz.api.url.getAdminLink(refModel, 'overview', doc._id.toString())}" title="${doc.title}">${doc.title}</a>`))
        .catch(callback);
