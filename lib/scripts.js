...
const parseDataAttributes = (scriptObj) => {

    // Don't mutate the original object.
    const script = Object.assign({}, scriptObj);

    if (!script.dataAttributes) {
        return script;
    }

    const dataAttributes = Object.keys(script.dataAttributes);
    const parsedAttributes = {};

    dataAttributes.forEach((attribute) => {
        parsedAttributes[`data-${attribute}`] = script.dataAttributes[attribute];
    });

    script.dataAttributes = parsedAttributes;

    return script;

};

const parseScripts = (scripts = []) => scripts.map(parseDataAttributes);
...
const setTemplateScripts = (req, res, templateScripts) => new Promise((resolve, reject) => {
    ...
    const defaults = [
        ...
        {
            src: `${linz.get('admin path')}/public/js/csrf-submit.js?v1`,
        },
    ];
    ...
    if (!customScripts) {
        return resolve(parseScripts(res.locals.scripts));
    }

    customScripts(req, res)
        .then((result) => resolve(parseScripts(result)))
        .catch(reject);
