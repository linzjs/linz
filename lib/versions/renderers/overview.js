...
module.exports = function versionsRenderer(
    req,
    res,
    record,
    model,
    ...
    async.waterfall(
        [
            ...
            async function() {
                const result = await model.VersionedModel.find(
                    { refId: record._id },
                    'dateCreated modifiedBy',
                    { lean: 1, sort: { dateCreated: -1 } }
                );
        ...
        function(err, records) {
            ...
            async.each(
                records,
                function(record, valuesDone) {
                    async.series(
                        [
                            function(getValueDone) {
                                settings.cellRenderers.date(
                                    record.dateCreated,
                                    record,
                                    'dateCreated',
                                    model,
                                    function(getValueErr, result) {
                                        if (getValueErr) {
                            ...
                            function(getValueDone) {
                                settings.cellRenderers.referenceName(
                                    record.modifiedBy,
                                    record,
                                    'modifiedBy',
                                    model,
                            ...
                            function(getValueDone) {
                                settings.cellRenderers.reference(
                                    record.modifiedBy,
                                    record,
                                    'modifiedBy',
                                    model,
                ...
                function(getRecordErr) {
                    ...
                    records.forEach(function(record, index) {
                        ...
                        records.forEach(function(historyRecord) {
                            if (
                                historyRecord.dateCreated < record.dateCreated
                            ) {
