 ...
 */
const generateForm = (model, options = {}) =>
    new Promise((resolve, reject) => {
        const {
            form: formOption,
            record = {},
        ...
        formAsPromise(req, model.modelName, formOption)
            .then((decoratedForm) =>
                linz.api.formtools.namespaceForm(req, decoratedForm)
            )
            .then((decoratedForm) => {
                // Generate labels
 ...
 * @return {String} Returns the form HTML.
 */
const generateFormString = (model, options = {}) =>
    new Promise((resolve, reject) => {
        const { actionUrl = '#', cancelUrl = '/', csrfToken } = options;

        generateForm(model, options)
            .then((form) =>
                linz.api.views.renderPartial(
                    'form',
                    {
                        actionUrl,
                        cancelUrl,
