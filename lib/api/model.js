...
function form (req, modelName, callback, formDsl) {
    ...
    let formFn = () => new Promise((resolve, reject) => {
...
const generateForm = (model, options = {}) => new Promise((resolve, reject) => {
    ...
    console.log('LINZ BEFORE', req.linz);

    formAsPromise(req, model.modelName, formOption)
        .then((decoratedForm) => {

            console.log('LINZ DURING', req.linz);

            return linz.api.formtools.namespaceForm(req, decoratedForm);
        ...
        .then((decoratedForm) => {
            ...
            // Generate the form
            linz.formtools.form.generateFormFromModel(model.schema, decoratedForm, record, type, (err, formObj) => {
