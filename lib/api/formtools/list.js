        ...
        async.each(
            ...
            function(item, itemDone) {
                if (!item.renderer) {
                    return itemDone(
                        new Error(
                            'Renderer function is missing for toolbar item: ' +
...
function renderFilters(req, modelName, cb) {
    ...
    linz.api.model.list(req, modelName, function(err, list) {
        ...
        async.each(
            ...
            function(fieldName, filtersDone) {
                // call the filter renderer and update the content with the result
                list.filters[fieldName].filter.renderer(fieldName, function(
                    filterErr,
                    result
                ) {
                    if (filterErr) {
...
function renderSearchFilters(req, fieldNames, data, modelName, cb) {
    ...
    if (!Array.isArray(fieldNames) || !fieldNames.length) {
        throw new Error(
            'fieldNames is required and must be of type Array and cannot be empty.'
        );
    }
    ...
    fieldNames = dedupe(
        fieldNames
            .sort()
            .filter((fieldName) => Object.keys(data).indexOf(fieldName) >= 0)
    );
    ...
    linz.api.model.list(req, modelName, function(err, list) {
        ...
        async.each(
            ...
            function(fieldName, filtersDone) {
                ...
                // call the filter renderer and update the content with the result
                list.filters[fieldName].filter.filter(fieldName, data, function(
                    filterErr,
                    result
                ) {
                    if (filterErr) {
    ...
    linz.api.model.list(req, modelName, function(err, list) {
        ...
        async.each(
            ...
            function(fieldName, filtersDone) {
                ...
                list.filters[fieldName].filter.bind(fieldName, data, function(
                    filterErr,
                    result
                ) {
                    if (filterErr) {
...
const getListFilters = (req) =>
    new Promise((resolve, reject) => {
        ...
        Model.getList(req, (err, list) => {
            ...
            Promise.all(
                filters.selectedFilters.split(',').map(
                    (fieldName) =>
                        new Promise(
                            (resolveSelectedFilters, rejectSelectedFilters) => {
                                // Call the filter renderer and update the content with the result
                                list.filters[fieldName].filter.filter(
                                    fieldName,
                                    filters,
                                    (err, result) => {
                                        if (err) {
                                        ...
                                        filterObj = Model.addSearchFilter(
                                            filterObj,
                                            result
                                        );
 ...
 */
const getSearchFilters = (req, currentFilters = {}) =>
    new Promise((resolve, reject) => {
        const filters = JSON.parse(req.body.filters);
        ...
        req.linz.model.getList(req, (err, list) => {
            ...
            Promise.all(
                list.search.map(
                    (field) =>
                        new Promise(
                            (resolveSearchFilters, rejectSearchFilters) => {
                                linz.api.model.titleField(
                                    req.linz.model.modelName,
                                    field,
                                    (searchFilterErr, titleField) => {
                                        if (searchFilterErr) {
                                            return rejectSearchFilters(
                                                searchFilterErr
                                            );
                                        }

                                        return resolveSearchFilters(
 ...
 * @returns {Promise} Resolves with the updates query object.
 */
const getIdFilters = (req, currentFilters = {}) =>
    new Promise((resolve) => {
        ...
        const ids = selectedIds
            .split(',')
            .map((id) => new linz.mongoose.Types.ObjectId(id));
