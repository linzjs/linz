...
/**
 * Given `req`, add the form DSL to `req` and to embedded document fields.
 * @param {Object} req A HTTP request object.
 */
const namespace = (req) => new Promise((resolve, reject) => {

    linz.api.model.form(req, req.linz.model.modelName, function(err, formObj) {

        if (err) {
            return reject(err);
        }

        req.linz.model.linz.formtools.form = formObj;
