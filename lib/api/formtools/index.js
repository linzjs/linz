...
const linz = require('../../../');
const { parseField, parseForm } = require('./parse-form');

/**
 * Given `req`, add the form DSL to `req`.
 * @param {Object} req A HTTP request object.
 */
const namespaceForm = (req, form) =>
 ...
 * @param {Object} req A HTTP request object.
 */
const namespaceOverview = (req) =>
    new Promise((resolve, reject) => {
        linz.api.model.overview(req, req.linz.model.modelName, function(
            err,
            overview
        ) {
            if (err) {
