 ...
 */
const parseDisabledProperties = (record, actions) =>
    new Promise((resolve, reject) => {
        const hasOwnProperty = Object.prototype.hasOwnProperty;
        const parsedActions = []
            .concat(actions)
            .filter((action) => hasOwnProperty.call(action, 'disabled'));
        ...
        parsedActions.forEach((action) => {
            if (typeof action.disabled !== 'function') {
                return reject(
                    new Error(
                        'Invalid type for overview.action.disabled. It must be a function.'
                    )
                );
            }
