...
const parseField = (fieldName, data, options = {}) => {
    ...
    const { fieldType, form, formType } = options;

    let updatedField = field;
    const type = fieldType.toLowerCase();

    if (Array.isArray(field) && !['array', 'documentarray'].includes(type)) {
        throw Error(
            `Expected field ${fieldName} is type of ${type}, but found type of array.`
        );
    }
    ...
    const formItem = Object.assign(
        {},
        form[fieldName],
        form[fieldName][formType]
    );

    if (formItem.widget && formItem.widget.transform) {
        updatedField = formItem.widget.transform(
            fieldName,
            formItem,
            field,
            data
 ...
 */
const parseForm = (model, req, formDSL, options = {}) =>
    new Promise((resolve, reject) => {
        model.getForm(req, (err, modelForm) => {
            ...
            const { data = req.body, formType } = options;
            ...
            Object.keys(data)
                ...
                .forEach((fieldName) => {
                    ...
                    const fieldType =
                        (formDSL[fieldName] &&
                            formDSL[fieldName].type &&
                            formDSL[fieldName].type.toLowerCase()) ||
                        'string';

                    parsedForm[fieldName] = parseField(fieldName, data, {
                        fieldType,
                        form,
                        formType,
                    });
