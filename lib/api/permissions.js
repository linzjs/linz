...
function hasPermission (user, context, permission, cb) {
	...
	if (context.type.toLowerCase() === 'model') {
		return linz.api.model.hasPermission(user, context.model, permission, cb);
	}

	if (context.type.toLowerCase() === 'config') {
		return linz.api.configs.hasPermission(user, context.config, permission, cb);
	}
