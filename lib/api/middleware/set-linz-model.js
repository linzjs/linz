...
const getModelFromRequest = (req) => {
    ...
    const modelMatch = req.path.match(/(?<=\/model\/)(\w*)(?<=\/\w*)/);

    // The match method returns null or an array of matches so we need to destructure it below.
    if (!modelMatch) {
        return null;
    }

    const [foundModel] = modelMatch;
