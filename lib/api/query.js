...
const field = (modelName, field, queryFor) => {
    ...
    if (!Object.keys(Model.schema.paths).includes(field)) {
        throw new Error(
            `The ${field} field could not be found in ${modelName}`
        );
    }
...
const regexp = (string, flags = 'ig') => ({
    $regex: stringToRegexp(string, flags),
});
...
const dateRangeQuery = (
    ...
    const $gte = isoString(fromDateString, dateFormat, offset, (date) =>
        date.startOf('day')
    );
    const $lte = isoString(toDateString, dateFormat, offset, (date) =>
        date.endOf('day')
    );
