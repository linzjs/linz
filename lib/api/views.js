...
const renderNotifications = (req, cb) => {
    ...
    return linz.api.views.renderPartial('notifications', { notifications: req.linz.notifications }, cb);
        ...
        attributesMiddleware(req, {}, () => {
            ...
            renderNotifications(req, (err, notifications) => {
                ...
                linz.api.navigation.get(req, (err, navigation) => {
                    ...
                    Promise.all([
                        getScripts(req, res, data.scripts),
                        getStyles(req, res, data.styles),
                    ])
                        .then(([scripts, styles]) => {

                            // Remove the passed in scripts and styles after using them so we can use the combined ones above.
                            if (data.scripts) {
                                delete data.scripts;
                            }
