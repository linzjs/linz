...
const setLinzNamespace = require('./middleware/set-linz-namespace');
 ...
 */
const getScripts = (req, res, scripts = []) =>
    setTemplateScripts(req, res, scripts);
 ...
 */
const getStyles = (req, res, styles = []) =>
    setTemplateStyles(req, res, styles);
...
/**
 * Give a request object, render any notifications within in it.
 * @param  {Object}   req A HTTP request object.
 * @param  {Function} cb  A callback to return the results too.
 * @return {Void}         Calls the callback with the results.
 */
const renderNotifications = (req, cb) => {
    // If there are none, return none.
    if (!req.linz.notifications.length) {
        return cb();
    }

    return linz.api.views.renderPartial(
        'notifications',
        { notifications: req.linz.notifications },
        cb
    );
};
        ...
        attributesMiddleware(req, {}, () => {
            renderNotifications(req, (err, notifications) => {
                ...
                linz.api.navigation.get(req, (err, navigation) => {
                    ...
                    Promise.all([getScripts(req, res), getStyles(req, res)])
                        .then(([scripts, styles]) => {
                            const appLocals = {
                            ...
                            const locals = utils.merge(
                                appLocals,
                                clone(data) || {}
                            );
                            const view = viewPath(locals.template + '.pug');

                            if (!view) {
                                return callback(
                                    new Error('Could not find template.')
                                );
                            }
...
const renderPartial = (view, locals, callback) => {
    return linz.app.render(
        linz.api.views.viewPath(`partials/${view}.pug`),
        locals,
        callback
    );
};
