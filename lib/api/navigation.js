...
function get(req, cb) {
    if (
        !linz.get('disable navigation cache') &&
        !req.linz.cache.navigation.invalidate &&
    ...
    var flatNavigation =
        linz.get('flat navigation') ||
        (function(nestedNavigation) {
            linz.set(
                'flat navigation',
                flattenNavigation(nestedNavigation, false)
            );
    ...
    async.seq(
        function(fn, callback) {
            async.filter(
                fn,
                function(navigationItem, filterCb) {
                    ...
                    if (typeof navigationItem.permission === 'function') {
                        return navigationItem.permission(req.user, function(
                            result
                        ) {
        ...
        function(filteredNavigation, callback) {
            ...
            Promise.resolve(
                linz.get('navigationTransform')(nestedNavigation, {
                    user: req.user,
                })
            ).then((navigation) => {
