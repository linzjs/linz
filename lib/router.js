...
exports.setupAdminRoutes = function (linz) {
	...
	router.post('*', helpers.linz(linz), middleware.adminEnsureAuthenticated());
	router.get('/', routes.adminHome);
	router.get('/login', routes.adminLogin.get);
	router.post('/login', passport.authenticate('linz-local', { failureRedirect: linz.get('admin path') + '/login', successRedirect: linz.get('admin path') }), routes.adminLogin.post);
