!(function($) {
    ...
    DocumentArray.prototype.drawDocuments = function(
        documentArrayInstance,
        documents
    ) {
        var da = this,
        ...
        if (
            $('[data-document-field-for="' + documentArrayInstance + '"]').attr(
                'data-document-set-label'
            ) !== undefined
        ) {
        ...
        $(
            '[data-document-field-for="' +
                documentArrayInstance +
                '"] .documents'
        ).html(this.listTemplate(documents));

        if (da.canEdit) {
            // now bind the documents, handle the edit button
            $('[data-document-field-for="' + documentArrayInstance + '"]')
                .find('a[data-document-action="edit"]')
                .click(function() {
                    $('#documentsModal .edit-actions .btn-save').unbind(
                        'click'
                    );
                    $('#documentsModal .edit-actions .btn-cancel').unbind(
                        'click'
                    );
                    $('#documentsModal .delete-actions .btn-save').unbind(
                        'click'
                    );
                    $('#documentsModal .delete-actions .btn-cancel').unbind(
                        'click'
                    );
                    $('#documentsModal button.close').unbind('click');

                    $('#documentsModal .edit-actions .btn-save').click(
                        function() {
                            da.saveAction();
                        }
                    );
                    $('#documentsModal .edit-actions .btn-cancel').click(
                        function() {
                            da.closeAction();
                        }
                    );
                    $('#documentsModal .delete-actions .btn-save').click(
                        function() {
                            da.deleteAction();
                        }
                    );
                    $('#documentsModal .delete-actions .btn-cancel').click(
                        function() {
                            da.closeAction();
                        }
                    );
                    $('#documentsModal button.close').click(function() {
                        da.closeAction();
                    });

                    da.editingFor = documentArrayInstance;

                    da.editingIndex = $(this)
                        .parent()
                        .attr('data-document-index');

                    // now grab the array if there already is one and turn it into a JavaScript object
                    da.editingArray = JSON.parse(
                        $(
                            '[data-document-field-for="' +
                                documentArrayInstance +
                                '"]'
        ...
        if (da.canDelete) {
            ...
            $('[data-document-field-for="' + documentArrayInstance + '"]')
                .find('a[data-document-action="remove"]')
                .click(function() {
                    $('#documentsModal .edit-actions .btn-save').unbind(
                        'click'
                    );
                    ...
                    da.editingObject = da.setLabel(
                        da.editingArray[da.editingIndex]
                    );
    ...
    DocumentArray.prototype.redrawDocuments = function(editingFor) {
        this.drawDocuments(
            editingFor,
            JSON.parse(
                $('input[type="hidden"][name="' + editingFor + '"]').val()
            )
        );
    };

    DocumentArray.prototype.editDocument = function(editingFor) {
        var form = $('#documentsModal .modal-body form').html(
            this.retrieveForm(editingFor)
        );

        var transforms = [];

        // Loop through each datetimepicker field and create a new transform so we get a proper date string.
        $('#documentsModal .modal-body form [data-linz-date-format]').each(
            function() {
                var field = $(this);
    ...
    DocumentArray.prototype.removeDocument = function(editingFor) {
        $('#documentsModal .modal-body form').html(
            Handlebars.compile(
                "Are you sure you would like to delete '{{label}}'?"
            )(this.editingObject)
        );
    ...
    DocumentArray.prototype.saveDocument = function() {
        ...
        // now grab the array if there already is one and turn it into a JavaScript object
        $('input[type="hidden"][name="' + this.editingFor + '"]').val(
            JSON.stringify(this.editingArray)
        );
    ...
    DocumentArray.prototype.saveAction = function() {
        var validator = $('#documentsModal .modal-body form').data(
            'bootstrapValidator'
        );
    ...
    DocumentArray.prototype.retrieveForm = function(editingFor) {
        if (!this.formHtml[editingFor]) {
            ...
            this.formHtml[editingFor] = window.linz.templates[
                'document-' + editingFor
            ]
                .clone()
                .html();
    ...
    DocumentArray.prototype.retrieveLabel = function(editingFor) {
        return $('[data-document-field-for="' + editingFor + '"]').attr(
            'data-document-field-label'
        );
    };
    ...
    $.fn.documentarray = function(option, parameter, extraOptions) {
        ...
        return this.each(function(index, el) {
            ...
            (data.canCreate =
                $(el).attr('data-document-can-create') !== undefined
                    ? true
                    : false),
                (data.canEdit =
                    $(el).attr('data-document-can-edit') !== undefined
                        ? true
                        : false),
                (data.canDelete =
                    $(el).attr('data-document-can-delete') !== undefined
                        ? true
                        : false);
            ...
            if (!isModalWired) {
                ...
                documentsModal.on('show.bs.modal', function() {
                    ...
                    $('[data-ui-datepicker]').each(function() {
                        ...
                        var values = JSON.parse(
                            documentsModal
                                .siblings('form')
                                .find('input[name="' + data.editingFor + '"]')
                                .val()
                        );
                        ...
                        var offset =
                            datepicker.data('utc-offset') ||
                            moment().format('Z');

                        datepicker
                            .data('DateTimePicker')
                            .date(
                                moment(dateValue).subtract(
                                    getOffset(moment().format('Z')) -
                                        getOffset(offset),
                                    'minutes'
                                )
                            );
                ...
                documentsModal.on('shown.bs.modal', function(e) {
                    ...
                    if (validator) {
                        validator.destroy();
                        $('#documentsModal .modal-body form').data(
                            'bootstrapValidator',
                            null
                        );
                    }
                ...
                var template = $('template.document-array-list')
                    .clone()
                    .html();
                ...
                if (!data.canEdit) {
                    ...
                    template = $(template)
                        .find('[data-document-action="edit"]')
                        .remove()
                        .end()[0].outerHTML;
                }
                ...
                if (!data.canDelete) {
                    ...
                    template = $(template)
                        .find('[data-document-action="remove"]')
                        .remove()
                        .end()[0].outerHTML;
                }
            ...
            if (data.canCreate) {
                ...
                $(
                    ...
                    var parentElem = $(event.target).closest(
                        '.documents-container'
                    );
                    ...
                    data.editingFor = $(parentElem).attr(
                        'data-document-field-for'
                    );

                    // now grab the array if there already is one and turn it into a JavaScript object
                    data.editingArray = JSON.parse(
                        $(parentElem)
                            .children(
                                'input[type="hidden"][name="' +
                                    data.editingFor +
