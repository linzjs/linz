...
(function() {
    ...
    $(document).ready(function() {
        ...
        var multiselectWidget = $('.multiselect').not(function(item, el) {
            return $(el).closest('span.controlField').length === 1
                ? true
                : false;
        });
        ...
        $('.ckeditor').each(function() {
            var editorConfig = {
                customConfig:
                    $(this).attr('data-linz-ckeditor-config') ||
                    adminPath + '/public/js/ckeditor-config-linz-default.js',
                contentsCss: $(this).attr('data-linz-ckeditor-style')
                    ? $(this)
                          .attr('data-linz-ckeditor-style')
                          .split(',')
                    : adminPath + '/public/css/ckeditor-linz-default.css',
            };

            // check if there are any widgets to include
            var widgets = $(this).attr('data-linz-ckeditor-widget')
                ? $(this)
                      .attr('data-linz-ckeditor-widget')
                      .split(',')
                : [];
        ...
        $('[data-linz-local-date]').each(function() {
            var dateFormat =
                $(this).attr('data-linz-date-format') || 'ddd DD/MM/YYYY';
            var localDateTime = moment(
                new Date($(this).attr('data-linz-utc-date'))
            ).format(dateFormat);
    ...
    function loadDatepicker() {
        if (
            $('[data-ui-datepicker]').length &&
            !$('[name="linzTimezoneOffset"]').length
        ) {
        ...
        if ($('[data-ui-datepicker]').length) {
            $('[data-ui-datepicker]').each(function() {
                ...
                var format =
                    field.attr('data-linz-date-format') || 'YYYY-MM-DD';
                var useCurrent =
                    field.attr('data-linz-date-use-current') === 'true';
                var sideBySide =
                    field.attr('data-linz-date-side-by-side') === 'true';
    ...
    function loadDatepickerDefaults() {
        if ($('[data-ui-datepicker]').length) {
            $('[data-ui-datepicker]').each(function() {
                ...
                var clientDateValue = offset
                    ? moment(dateValue).utcOffset(offset)
                    : moment(dateValue);
